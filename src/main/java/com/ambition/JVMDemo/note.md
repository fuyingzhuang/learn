...
- 2.为对象分配内存(首选计算对象占用空间大小，接着在堆中划分一块内存给新对象 如果实例成员变量是引用变量仅分配引用空间即可即4个字节大小)
- 2.1 如果内存是规整的 可以使用指针碰撞的方式分配内存(指针碰撞的前提是堆中的内存是绝对规整的 即所有用过的内存放在一边 未用的内存放在另一边 中间放着一个指针作为分界点的指示器 称为指针碰撞)
- 2.2 如果内存是不规整的 那么就需要使用空闲列表的方式进行内存分配(空闲列表的前提是堆内存是不规整的 即已使用的内存和未使用的内存相互交错 这时候就需要维护一个列表 记录哪些内存是可以使用的 哪些内存是已经被使用的)
- 3.处理并发安全问题(如果是多线程环境下)
- 采用CAS配上失败重试的方式保证更新的原子性 
- TLAB预先为每个线程在Eden分配一小块内存 提升对象分配的效率
- 4.初始化分配到的空间 默认初始化(零值填充) 如果是对象的实例变量是引用类型 则初始化为null 如果是基本数据类型 则初始化为0 保证对象在不赋值的情况下可以直接使用
- 5.设置对象头 将对象的所属类(即类的元数据信息) 对象的HashCode和对象的GC信息 锁信息等数据存储在对象的对象头中 这个过程的具体设置方式取决于JVM实现
- 6.执行init方法 进行初始化工作 在Java程序的视角来看 初始化才正式开始 初始化成员变量 执行实例话代码块 调用类的构造方法 并把堆内对象的首地址赋值给引用变量
