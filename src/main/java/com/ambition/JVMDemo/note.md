...
## 运行时数据区
- 程序计数器：
  也称为PC寄存器，是一块较小的内存空间，它可以看作是当前线程所执行的字节码的行号指示器，字节码解释器工作时就是通过改变这个计数器的值来选取下一条需要执行的字节码指令，它是线程私有的，生命周期与线程相同。
- Java虚拟机栈：每个Java方法在执行的同时会创建一个栈帧用于存储局部变量表、操作数栈、动态链接、方法出口等信息
- Java虚拟机栈（Java Virtual Machine Stack）是Java虚拟机的一部分，用于存储线程的方法调用和局部变量。每个线程在执行Java方法时都会有一个对应的栈，用于跟踪方法的执行和管理局部变量。在Java虚拟机规范中，对Java虚拟机栈的描述为： 每个线程创建时都会被分配一个Java虚拟机栈，该栈的大小在创建时可以指定，也可以动态扩展。
Java虚拟机栈由多个栈帧（Stack Frame）组成，每个栈帧对应一个方法的执行。
栈帧包含了局部变量表、操作数栈、动态链接、方法返回地址等信息。
每个方法在执行时会创建一个栈帧并压入Java虚拟机栈顶，方法执行结束后栈帧出栈。
如果线程请求的栈深度大于Java虚拟机所允许的深度，就会抛出StackOverflowError异常。
如果Java虚拟机栈可以动态扩展，但在扩展时没有足够的内存，就会抛出OutOfMemoryError异常。
Java虚拟机栈与本地方法栈（Native Method Stack）类似，不过本地方法栈是为执行Native方法（使用Native关键字声明，并由本地代码实现的方法）服务的。两者的区别在于Java虚拟机栈执行的是Java方法，而本地方法栈执行的是本地方法。
